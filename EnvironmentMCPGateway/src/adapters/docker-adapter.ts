...
export class DockerAdapter {
    ...
    private parseDockerPsOutput(output: string): ContainerInfo[] {
        ...
        }).filter(container => container !== null) as ContainerInfo[];
    }
    ...
    async listDevelopmentContainers(): Promise<ContainerInfo[]> {
    ...
    async getContainerHealth(containerId: string): Promise<{ healthy: boolean; message: string }> {
        try {
    ...
    async getTimescaleDBStatus(): Promise<DatabaseContainerStatus> {
        const containers = await this.listDevelopmentContainers();
        const timescaleContainer = containers.find(c => 
            DockerAdapter.TIMESCALE_IMAGE_PATTERNS.some(pattern => c.image.toLowerCase().includes(pattern))
        );
        ...
        let health: 'healthy' | 'degraded' | 'failed' = 'failed';
        let message = '';
    ...
    async getRedPandaStatus(): Promise<MessageQueueContainerStatus> {
        const containers = await this.listDevelopmentContainers();
        const redpandaContainer = containers.find(c => 
            DockerAdapter.REDPANDA_IMAGE_PATTERNS.some(pattern => c.image.toLowerCase().includes(pattern))
        );
        const consoleContainer = containers.find(c => 
            DockerAdapter.REDPANDA_CONSOLE_IMAGE_PATTERNS.some(pattern => c.image.toLowerCase().includes(pattern))
        );
    ...
    private evaluateContainerHealth(container: ContainerInfo | undefined): 'healthy' | 'degraded' | 'failed' {
    ...
    async getDevelopmentEnvironmentHealth(): Promise<EnvironmentHealth> {
        const [databaseStatus, messagingStatus] = await Promise.all([
            this.getTimescaleDBStatus(),
            this.getRedPandaStatus()
        ]);

        const issues: Array<{ component: string; severity: 'warning' | 'error'; message: string }> = [];
        const recommendations: Array<{ issue: string; suggestion: string; command?: string }> = [];
