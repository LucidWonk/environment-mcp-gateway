...
export const expertSelectWorkflowTool = {
...
export const agentCoordinateHandoffTool = {
...
export const workflowClassifyTool = {
...
export const expertStatusMonitorTool = {
...
export const expertConflictResolveTool = {
...
export const expertValidateImplementationTool = {
...
export const expertGetProjectStandardsTool = {
...
export class ExpertSelectionEngine {
    static analyzeComponents(filePaths) {
        const domains = [];
        const riskFactors = [];
    ...
    static analyzeWorkflowDescription(description) {
    ...
    static selectExperts(workflowDescription, filePaths = [], providedWorkflowType, providedRiskLevel) {
        ...
        if (providedWorkflowType)
            confidence += 0.1;
        if (providedRiskLevel)
            confidence += 0.1;
        if (filePaths.length > 0)
            confidence += 0.1;
        if (workflowAnalysis.keywords.length >= 3)
            confidence += 0.1;
...
export class AgentCoordinationManager {
    static async coordinateHandoff(primaryAgentContext, secondaryExpertType, subtaskDescription, contextScope = 'focused') {
    ...
    static generateContextHash(context) {
    ...
    static extractRelevantContext(fullContext, subtask) {
    ...
    static generateContextSummary(context) {
        // Generate a brief summary of the context
        const lines = context.split('\n').filter(line => line.trim().length > 0);
        return lines.slice(0, 3).join('. ') + (lines.length > 3 ? '...' : '');
    }
    static async generateCoordinationInstructions(expertType, subtask) {
        try {
    ...
    static estimateDuration(expertType, subtask, contextScope) {
...
export const virtualExpertTeamHandlers = {
    'expert-select-workflow': async (args) => {
        try {
            ...
            const selection = ExpertSelectionEngine.selectExperts(args.workflowDescription, args.filePaths || [], args.workflowType, args.riskLevel);
            ...
            const expertAssignment = await taskToolVETIntegration.assignExperts(taskId, args.workflowDescription, selection);
            const response = {
                success: true,
                taskId,
    ...
    'agent-coordinate-handoff': async (args) => {
        try {
            ...
            const coordination = await AgentCoordinationManager.coordinateHandoff(args.primaryAgentContext, args.secondaryExpertType, args.subtaskDescription, args.contextScope || 'focused');
    ...
    'workflow-classify': async (args) => {
        try {
            ...
            const selection = ExpertSelectionEngine.selectExperts(args.workflowDescription, args.componentPaths || []);
            const componentAnalysis = ExpertSelectionEngine.analyzeComponents(args.componentPaths || []);
            const workflowAnalysis = ExpertSelectionEngine.analyzeWorkflowDescription(args.workflowDescription);
    ...
    'expert-status-monitor': async (args) => {
        try {
    ...
    'expert-conflict-resolve': async (args) => {
        try {
    ...
    'expert-validate-implementation': async (args) => {
        try {
            ...
            const response = {
                ...
                expertAnalysis: args.expertRecommendations?.map((rec) => ({
    ...
    'expert-get-project-standards': async (args) => {
        try {
...
//# sourceMappingURL=virtual-expert-team.js.map