 ...
 */
export const analyzedomainmapTool = {
...
export const predictchangeimpactTool = {
...
export const coordinatecrossdomainupdateTool = {
...
export const analyzespecificdomainsimpactTool = {
...
export const getcrossdomaincoordinationstatusTool = {
 ...
 */
export async function handleAnalyzeDomainMap(args) {
    ...
    try {
        ...
        if (includeVisualization) {
            response.domainMap.visualization = {
...
export async function handlePredictChangeImpact(args) {
    const { changedFiles, projectRoot = '.', includeRiskAnalysis = true, includeRecommendations = true } = args;
    try {
        ...
        if (includeRiskAnalysis) {
            response.prediction.riskAnalysis = {
        ...
        if (includeRecommendations) {
            response.prediction.recommendations = prediction.recommendations;
        }
        return response;
    }
    catch (error) {
...
export async function handleCoordinateCrossDomainUpdate(args) {
    const { changedFiles, triggerType = 'manual', performanceTimeout = 300, projectRoot = '.', dryRun = false } = args;
    try {
...
export async function handleAnalyzeSpecificDomainsImpact(args) {
    const { sourceDomains, targetDomains, projectRoot = '.', includeTransitiveImpacts = true, maxPropagationDepth = 4 } = args;
    try {
        const domainAnalyzer = new DomainAnalyzer(projectRoot);
        ...
        const relevantRelationships = domainMap.relationships.filter(rel => sourceDomains.includes(rel.sourceDomain) ||
            (targetDomains && targetDomains.includes(rel.targetDomain)));
        // Build impact analysis
        const directImpacts = new Set();
        const transitiveImpacts = new Map(); // domain -> depth
        // Add direct impacts
...
export async function handleGetCrossDomainCoordinationStatus(args) {
 ...
 */
export function getCrossDomainImpactAnalysisTools() {
    return [
 ...
 */
export function getCrossDomainImpactAnalysisHandlers() {
    return new Map([
...
function findPropagationPaths(sourceDomain, relationships, maxDepth, currentPath = [], visited = new Set()) {
...
//# sourceMappingURL=cross-domain-impact-analysis.js.map