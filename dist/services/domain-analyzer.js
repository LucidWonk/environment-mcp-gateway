...
export class DomainAnalyzer {
    ...
    performanceThreshold = 10000; // 10 seconds default
    constructor(projectRoot = '.', timeout = 10000) {
     ...
     */
    async analyzeDomainMap(changedFiles) {
     ...
     */
    async discoverDomainBoundaries(analysisStartTime) {
     ...
     */
    async analyzeDomainBoundary(domainName, patterns, analysisStartTime) {
     ...
     */
    async findDomainFiles(patterns, analysisStartTime) {
        const matchingFiles = [];
     ...
     */
    async recursiveFileSearch(directory, patterns) {
        const matchingFiles = [];
        try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
                    ...
                    const matches = patterns.some(pattern => pattern.test(relativePath) ||
     ...
     */
    extractBusinessConcepts(semanticResults) {
     ...
     */
    extractBusinessRules(semanticResults) {
        const rules = new Set();
        for (const result of semanticResults) {
     ...
     */
    extractKeyInterfaces(semanticResults, domainFiles) {
     ...
     */
    identifySubDomains(domainFiles, domainName) {
     ...
     */
    determineDomainRootPath(domainFiles, domainName) {
        if (domainFiles.length === 0) {
     ...
     */
    calculateDomainConfidence(domainFiles, semanticResults, patterns) {
        let confidence = 0;
        ...
        if (semanticResults.length > 0) {
            const avgSemanticConfidence = semanticResults.reduce((sum, result) => sum + (result.businessConcepts.reduce((conceptSum, concept) => conceptSum + concept.confidence, 0) / Math.max(result.businessConcepts.length, 1)), 0) / semanticResults.length;
            confidence += avgSemanticConfidence * 0.3;
        }
        // Directory structure coherence
        const hasCoherentStructure = domainFiles.some(file => path.dirname(file).split(path.sep).length >= 3);
     ...
     */
    async discoverUnknownDomains(knownBoundaries) {
     ...
     */
    async analyzeRelationships(boundaries, changedFiles, analysisStartTime) {
        ...
        for (let i = 0; i < boundaries.length; i++) {
            for (let j = i + 1; j < boundaries.length; j++) {
                ...
                const relationship = await this.analyzeRelationshipBetween(sourceDomain, targetDomain, changedFiles);
                if (relationship) {
                    relationships.push(relationship);
                    // If bidirectional, create reverse relationship
                    if (relationship.strength > 0.6) {
                        const reverseRelationship = await this.analyzeRelationshipBetween(targetDomain, sourceDomain, changedFiles);
     ...
     */
    async analyzeRelationshipBetween(sourceDomain, targetDomain, _changedFiles) {
     ...
     */
    async findCrossDomainReferences(sourceDomain, targetDomain) {
        const evidenceFiles = [];
        try {
     ...
     */
    checkForCrossDomainReference(content, targetDomain) {
     ...
     */
    determineRelationshipType(sourceDomain, targetDomain, evidenceFiles) {
        // Analyze evidence patterns
        const hasDataReference = evidenceFiles.some(file => targetDomain.domain.toLowerCase().includes('data') ||
            file.toLowerCase().includes('repository') ||
            file.toLowerCase().includes('database'));
        if (hasDataReference) {
            return 'dependency';
        }
        const hasServiceReference = evidenceFiles.some(file => file.toLowerCase().includes('service') ||
            file.toLowerCase().includes('handler'));
        if (hasServiceReference) {
            return 'association';
        }
        const hasInheritancePattern = evidenceFiles.some(_file => sourceDomain.domain.includes(targetDomain.domain) ||
     ...
     */
    calculateRelationshipStrength(sourceDomain, targetDomain, evidenceFiles) {
        let strength = 0;
        // File count factor
        ...
        const conceptOverlap = sourceDomain.businessConcepts.filter(concept => targetDomain.businessConcepts.includes(concept)).length;
     ...
     */
    generateBusinessJustification(sourceDomain, targetDomain, relationshipType) {
     ...
     */
    identifyIsolatedDomains(boundaries, relationships) {
     ...
     */
    async identifyCrossCuttingConcerns(analysisStartTime) {
     ...
     */
    async analyzeChangedFileImpacts(changedFiles) {
        ...
        for (const domain of initialDomains) {
            const relatedRelationships = domainMap.relationships.filter(rel => rel.sourceDomain === domain || rel.targetDomain === domain);
        ...
        const propagationPaths = this.buildPropagationPaths(initialDomains, Array.from(affectedDomains), domainMap.relationships);
        return {
     ...
     */
    buildPropagationPaths(initialDomains, allAffectedDomains, relationships) {
        const paths = [];
        for (const initialDomain of initialDomains) {
            const visited = new Set();
            const currentPath = [initialDomain];
            this.findPropagationPaths(initialDomain, relationships, visited, currentPath, paths, 3 // Max depth
            );
        }
        return paths;
    }
     ...
     */
    findPropagationPaths(currentDomain, relationships, visited, currentPath, allPaths, maxDepth) {
        if (currentPath.length > maxDepth) {
            return;
        }
        visited.add(currentDomain);
        const outgoingRelationships = relationships.filter(rel => rel.sourceDomain === currentDomain && !visited.has(rel.targetDomain));
        ...
        for (const relationship of outgoingRelationships) {
            ...
            this.findPropagationPaths(relationship.targetDomain, relationships, new Set(visited), newPath, allPaths, maxDepth);
        }
    }
    /**
