...
export class ImpactMapper {
    ...
    performanceThreshold = 15000; // 15 seconds default
    maxPropagationDepth = 5; // Maximum depth for impact propagation
    constructor(projectRoot = '.', timeout = 15000) {
     ...
     */
    async predictChangeImpact(changedFiles) {
     ...
     */
    async getDomainMap(changedFiles) {
     ...
     */
    async buildImpactGraph(changedFiles, domainMap, analysisStartTime) {
        const impactNodes = [];
        const impactEdges = [];
        ...
        for (const domain of directlyAffectedDomains) {
            ...
            if (!domainBoundary)
                continue;
            const domainChangedFiles = changedFiles.filter(file => file.startsWith(domainBoundary.rootPath));
            const node = await this.createImpactNode(domain, 'direct', domainChangedFiles, domainBoundary);
        ...
        await this.propagateImpact(directlyAffectedDomains, domainMap, impactNodes, impactEdges, 1, // Starting depth
        analysisStartTime);
     ...
     */
    identifyDirectlyAffectedDomains(changedFiles, domainMap) {
     ...
     */
    async createImpactNode(domain, impactLevel, changedFiles, domainBoundary, propagationDepth = 0) {
        ...
        const impactScore = this.calculateImpactScore(impactLevel, changedFiles.length, semanticResults, domainBoundary, propagationDepth);
        // Identify affected components
        const affectedComponents = this.identifyAffectedComponents(semanticResults, domainBoundary);
        // Estimate update time
        const estimatedUpdateTime = this.estimateUpdateTime(impactLevel, changedFiles.length, affectedComponents.length, domainBoundary.businessConcepts.length);
     ...
     */
    async analyzeSemanticChanges(changedFiles, domainBoundary) {
     ...
     */
    calculateImpactScore(impactLevel, changedFileCount, semanticResults, domainBoundary, propagationDepth) {
        let score = 0;
        ...
        if (semanticResults.length > 0) {
            const avgComplexity = semanticResults.reduce((sum, result) => sum + result.businessConcepts.length + result.businessRules.length, 0) / semanticResults.length;
     ...
     */
    identifyAffectedComponents(semanticResults, domainBoundary) {
     ...
     */
    estimateUpdateTime(impactLevel, changedFileCount, componentCount, businessConceptCount) {
     ...
     */
    determineUpdatePriority(impactScore, propagationDepth, domain) {
     ...
     */
    async propagateImpact(sourceDomainsToProcess, domainMap, impactNodes, impactEdges, currentDepth, analysisStartTime, maxDepth = 4) {
        ...
        const nextLevelDomains = new Set();
        ...
        for (const sourceDomain of sourceDomainsToProcess) {
            ...
            const outgoingRelationships = domainMap.relationships.filter(rel => rel.sourceDomain === sourceDomain);
            for (const relationship of outgoingRelationships) {
                ...
                if (!processedNodes.has(targetDomain)) {
                    ...
                    if (targetBoundary) {
                        ...
                        const node = await this.createImpactNode(targetDomain, impactLevel, [], // No direct file changes
                        targetBoundary, currentDepth);
                        impactNodes.push(node);
        ...
        if (nextLevelDomains.size > 0) {
            await this.propagateImpact(Array.from(nextLevelDomains), domainMap, impactNodes, impactEdges, currentDepth + 1, analysisStartTime, maxDepth);
        }
    }
    /**
     * Create an impact edge from a cross-domain relationship
     */
    createImpactEdge(relationship, propagationDepth) {
     ...
     */
    calculateUpdateSequence(impactGraph) {
     ...
     */
    async identifyRiskFactors(impactGraph, _domainMap) {
        const riskFactors = [];
     ...
     */
    detectCircularDependencies(impactGraph) {
     ...
     */
    detectHighCoupling(impactGraph) {
     ...
     */
    detectPerformanceRisks(impactGraph) {
     ...
     */
    assessRollbackComplexity(impactGraph) {
     ...
     */
    calculateConfidenceScore(impactGraph, domainMap) {
        let confidence = 0;
        // Domain map quality factor
        const avgDomainConfidence = domainMap.domains.reduce((sum, domain) => sum + domain.confidence, 0) / domainMap.domains.length;
        confidence += avgDomainConfidence * 0.4;
        // Relationship quality factor
        const avgRelationshipStrength = domainMap.relationships.reduce((sum, rel) => sum + rel.strength, 0) / Math.max(domainMap.relationships.length, 1);
     ...
     */
    generateRecommendations(impactGraph, riskFactors) {
        const recommendations = [];
        // Critical path recommendations
        if (impactGraph.criticalPath.length > 0) {
            recommendations.push(`Focus on critical path: ${impactGraph.criticalPath.join(' -> ')} for optimal update sequencing`);
        }
        // High-priority domain recommendations
        const criticalNodes = impactGraph.nodes.filter(n => n.updatePriority === 'critical');
        if (criticalNodes.length > 0) {
            recommendations.push(`Prioritize critical domains: ${criticalNodes.map(n => n.domain).join(', ')}`);
        }
        // Performance recommendations
        if (impactGraph.totalEstimatedTime > 600) {
            recommendations.push('Consider parallel execution for independent domains to reduce total update time');
        }
        ...
        if (impactGraph.nodes.length > 5) {
            recommendations.push('Create comprehensive rollback snapshots before executing updates');
        }
     ...
     */
    checkTimeout(startTime, context) {
