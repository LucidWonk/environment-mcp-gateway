...
export var XmlRuleType;
(function (XmlRuleType) {
    XmlRuleType["ValidationRule"] = "validation";
    XmlRuleType["ConstraintRule"] = "constraint";
    XmlRuleType["CalculationRule"] = "calculation";
    XmlRuleType["ConfigurationRule"] = "configuration";
    XmlRuleType["BusinessLogicRule"] = "business-logic";
    XmlRuleType["AlgorithmRule"] = "algorithm";
    XmlRuleType["PerformanceRule"] = "performance";
})(XmlRuleType || (XmlRuleType = {}));
/**
...
export class XmlDocumentationRuleParser {
     ...
     */
    async parseXmlDocumentation(filePath) {
     ...
     */
    parseXmlComment(comment, filePath) {
        const rules = [];
     ...
     */
    applyPatterns(comment, patterns, ruleType, filePath, baseConfidence) {
        const rules = [];
     ...
     */
    extractAttributeRules(content, filePath) {
        const rules = [];
     ...
     */
    extractIndicatorParameterRules(content, filePath) {
        const rules = [];
     ...
     * Step 3.2: C# Attribute Metadata Analysis
     */
    extractIndicatorDefinitionRules(content, filePath) {
        const rules = [];
     ...
     * Step 3.2: C# Attribute Metadata Analysis
     */
    extractValidationAttributeRules(content, filePath) {
        const rules = [];
     ...
     */
    extractAttributeCombinationRules(content, filePath) {
        const rules = [];
     ...
     */
    extractPropertyAttributeRules(content, filePath) {
        const rules = [];
     ...
     */
    analyzeParameterSemantics(attributeContent, propertyType, propertyName, filePath) {
        const rules = [];
     ...
     */
    analyzeIndicatorDefinitionSemantics(attributeContent, className, filePath) {
        const rules = [];
     ...
     */
    calculateAttributeConfidence(attributeType, attributeContent) {
     ...
     */
    extractConstraintsFromAttribute(attributeContent) {
        const constraints = [];
     ...
     */
    parseIndicatorParameterAttribute(attributeContent, filePath) {
     ...
     */
    parseIndicatorDefinitionAttribute(attributeContent, filePath) {
        const nameMatch = attributeContent.match(/"([^"]+)"/);
        if (!nameMatch)
            return null;
        return {
     ...
     */
    extractValidationMethodRules(content, filePath) {
        const rules = [];
     ...
     */
    inferValidationRule(codePattern) {
     ...
     * BR-CEE-011: Achieve >80% accuracy in business rule classification
     */
    calculateRuleConfidence(ruleText, ruleType, baseConfidence) {
     ...
     * BR-CEE-012: Focus on semantic value for AI development assistance
     */
    generateRuleDescription(ruleText, ruleType) {
        switch (ruleType) {
     ...
     * BR-CEE-011: Achieve >80% accuracy in business rule classification
     */
    calculateClassificationAccuracy(rules) {
        if (rules.length === 0)
            return 0;
     ...
     */
    async enhanceSemanticAnalysisWithXmlRules(semanticResult, xmlResults) {
        const enhancedRules = [...semanticResult.businessRules];
     ...
     */
    mapXmlRuleTypeToBusinessRuleType(xmlType) {
        switch (xmlType) {
     ...
     */
    findRelatedConcepts(xmlRule, concepts) {
...
//# sourceMappingURL=xml-documentation-rule-parser.js.map