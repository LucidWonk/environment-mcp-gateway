...
const logger = createMCPLogger('semantic-analysis.log');
export class SemanticAnalysisService {
     ...
     */
    analyzeTradingSpecificConcepts(pathLower, concepts) {
     ...
     */
    getSophisticatedAlgorithmPatterns() {
        return [
     ...
     */
    matchesAlgorithmPattern(fileName, directoryPath, pattern) {
     ...
     */
    detectMathematicalComplexity(fileName, indicators) {
     ...
     */
    analyzeBusinessLogicComplexity(fileName, directoryPath, indicators) {
        let complexityBonus = 0.0;
     ...
     */
    analyzeTradingAlgorithmComplexity(fileName, directoryPath, indicators) {
        let complexityBonus = 0.0;
     ...
     */
    calculateAccuracyMetrics(analysisResults, expertFeedback) {
        ...
        const domainSpecificAccuracy = new Map();
        ...
        const confidenceDistribution = confidenceRanges.map(threshold => expertFeedback.filter(f => f.expertConfidence >= threshold).length / Math.max(expertFeedback.length, 1));
        return {
            overallAccuracy,
...
## Confidence Distribution
${metrics.confidenceDistribution.map((dist, idx) => `- **≥${(0.5 + idx * 0.1).toFixed(1)}**: ${(dist * 100).toFixed(1)}%`).join('\n')}
...
Generated: ${new Date().toISOString()}
    ...
    adaptivelyTuneConfiguration() {
        ...
        const tradingFeedback = recentFeedback.filter(f => tradingDomains.some(domain => f.expectedDomain.startsWith(domain)));
