        ...
        describe('submitResponse', () => {
            ...
            it('should handle expired workflows', async () => {
                const expiredRequest = {
                    ...sampleRequest,
                    priority: 'urgent' // 4 hour expiration
                };
                ...
                await expect(approvalWorkflow.submitResponse(approval.approvalId, {
                    approver: 'approver1',
                    decision: 'approved'
    ...
    describe('ArchiveManager', () => {
        ...
        describe('archiveDocument', () => {
            it('should archive document with metadata', async () => {
                ...
                mockFs.promises.writeFile.mockResolvedValue(undefined);
                mockFs.promises.unlink.mockResolvedValue(undefined);
                const result = await archiveManager.archiveDocument('Documentation/NewConcepts/test.md', 'Implemented/20250101-test.md', ['Documentation/Analysis/test.domain.req.md'], {
                ...
                expect(mockFs.promises.writeFile).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('<!-- ARCHIVE METADATA'), 'utf-8');
                // Verify original was deleted
                expect(mockFs.promises.unlink).toHaveBeenCalledWith(expect.stringContaining('test.md'));
            });
            it('should handle archive failures', async () => {
                mockFs.promises.access.mockRejectedValue(new Error('File not found'));
                await expect(archiveManager.archiveDocument('nonexistent.md', 'Implemented/test.md', [])).rejects.toThrow('Archive operation failed: File not found');
            });
        });
...
${originalContent}`;
                ...
                await archiveManager.archiveDocument('Documentation/test.md', 'Implemented/test.md', []);
                // Then retrieve it
    ...
    describe('Integration Tests', () => {
        it('should complete full document lifecycle', async () => {
            ...
            expect(result.updatedCapabilityIds).toEqual(expect.arrayContaining([
                expect.objectContaining({
                    placeholder: 'TEMP-ANALYSIS-FRACTAL-a7b3',
...
//# sourceMappingURL=document-lifecycle.test.js.map