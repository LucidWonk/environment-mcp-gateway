...
describe('RollbackManager', () => {
    ...
    describe('Holistic Snapshot Creation', () => {
        test('should create snapshot for single domain', async () => {
            ...
            const rollbackData = await rollbackManager.createHolisticSnapshot(updateId, affectedDomains, testDir);
    ...
    async function createTestRollback(updateId, timestamp, domains, status = 'pending') {
        const stateDir = path.join(testDir, 'state');
        const snapshotDir = path.join(testDir, 'snapshots');
        ...
        await fs.promises.writeFile(path.join(stateDir, `${updateId}.rollback.json`), JSON.stringify(stateInfo, null, 2));
        await fs.promises.writeFile(path.join(snapshotDir, `${updateId}.snapshot.json`), JSON.stringify(rollbackData, null, 2));
    }
});
//# sourceMappingURL=rollback-manager.test.js.map