...
describe('TimeoutManager', () => {
    ...
    describe('Basic Operation Execution', () => {
        test('should execute successful operation within timeout', async () => {
            ...
            const result = await timeoutManager.executeWithTimeout(mockOperation, 'singleFileAnalysis', { test: true });
        ...
        test('should timeout operation that exceeds limit', async () => {
            ...
            await expect(timeoutManager.executeWithTimeout(mockOperation, 'singleFileAnalysis', { test: true }, 500 // 500ms timeout
        ...
        test('should handle operation that throws error', async () => {
            ...
            await expect(timeoutManager.executeWithTimeout(mockOperation, 'singleFileAnalysis', { test: true })).rejects.toThrow('Operation failed');
        });
    });
    describe('Dynamic Timeout Scaling', () => {
        test('should scale timeout for large file counts in semantic analysis', async () => {
            ...
            const result = await timeoutManager.executeWithTimeout(mockOperation, 'semanticAnalysis', { fileCount: 100 } // Large file count should scale timeout
            );
            expect(result).toBe('success');
        ...
        test('should scale timeout for multiple domains in context generation', async () => {
            ...
            const result = await timeoutManager.executeWithTimeout(mockOperation, 'contextGeneration', { domainCount: 10 } // Many domains should scale timeout
            );
            expect(result).toBe('success');
    ...
    describe('Operation Monitoring', () => {
        test('should track active operations', async () => {
            ...
            const operationPromise = timeoutManager.executeWithTimeout(longOperation, 'semanticAnalysis', { test: true });
        ...
        test('should cancel all active operations', async () => {
            const longOperations = [
                new Promise((resolve) => setTimeout(() => resolve('op1'), 5000)),
                new Promise((resolve) => setTimeout(() => resolve('op2'), 5000)),
                new Promise((resolve) => setTimeout(() => resolve('op3'), 5000))
            ];
            // Start multiple operations
            const promises = longOperations.map(op => timeoutManager.executeWithTimeout(op, 'semanticAnalysis', { test: true }));
    ...
    describe('Performance Statistics', () => {
        ...
        test('should generate recommendations for high load', async () => {
            ...
            const operations = Array(6).fill(null).map((_, i) => new Promise((resolve) => setTimeout(() => resolve(`op${i}`), 3000)));
            const promises = operations.map((op, i) => timeoutManager.executeWithTimeout(op, 'semanticAnalysis', { operationId: i }));
            // Get stats while operations are running
            const stats = timeoutManager.getPerformanceStats();
            expect(stats.activeOperations).toBe(6);
            expect(stats.recommendations.some((r) => r.includes('concurrent operations'))).toBe(true);
            // Clean up
            timeoutManager.cancelAllOperations('Test cleanup');
        }, 1000);
    });
    describe('Error Scenarios', () => {
        test('should handle timeout in the middle of scaled operation', async () => {
            ...
            await expect(timeoutManager.executeWithTimeout(mockOperation, 'singleFileAnalysis', { test: true }, 1000 // 1 second timeout - should fail
        ...
        test('should provide helpful timeout suggestions', async () => {
            ...
            try {
                await timeoutManager.executeWithTimeout(mockOperation, 'contextGeneration', { fileCount: 100 }, 500);
        ...
        test('should handle concurrent timeout and success scenarios', async () => {
            ...
            const [fastResult] = await Promise.allSettled([
                timeoutManager.executeWithTimeout(fastOperation, 'singleFileAnalysis', { operation: 'fast' }),
                timeoutManager.executeWithTimeout(slowOperation, 'singleFileAnalysis', { operation: 'slow' }, 500 // This should timeout
                )
            ]);
    ...
    describe('Memory Management', () => {
        test('should clean up completed operations from memory', async () => {
            const operations = Array(5).fill(null).map((_, i) => new Promise((resolve) => setTimeout(() => resolve(`result${i}`), 100 + i * 50)));
            await Promise.all(operations.map((op, i) => timeoutManager.executeWithTimeout(op, 'singleFileAnalysis', { operationId: i })));
