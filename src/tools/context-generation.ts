 ...
 */
export const generateContextFilesTool: Tool = {
...
export async function handleGenerateContextFiles(args: any): Promise<any> {
    ...
    try {
        ...
        const domainResults = targetDomain === 'Unknown' 
            ...
            : analysisResults.filter(result => 
                result.domainAnalysis.primaryDomain === targetDomain ||
 ...
 */
export const previewContextFilesTool: Tool = {
...
export async function handlePreviewContextFiles(args: any): Promise<any> {
    const startTime = Date.now();
    ...
    try {
        ...
        preview.summary = {
            totalContentSize: contextFiles.reduce((sum, cf) => 
                sum + cf.domainOverview.length + cf.currentImplementation.length + 
 ...
 */
export const validateContextFilesTool: Tool = {
...
export async function handleValidateContextFiles(args: any): Promise<any> {
    const startTime = Date.now();
