...
describe('Full Workflow Integration Tests', () => {
    ...
    let orchestrator: HolisticUpdateOrchestrator;
    ...
    describe('End-to-End Context Generation', () => {
        ...
        test('should handle multiple domain changes', async () => {
            ...
            expect(result.affectedDomains).toEqual(
                expect.arrayContaining(['Analysis', 'Data', 'Messaging'])
            );
    ...
    describe('Error Handling and Recovery', () => {
        ...
        test('should clean up rollbacks automatically', async () => {
            ...
            const rollbackManager = new RollbackManager(
                path.join(testProjectRoot, '.test-rollback'),
                { 
            ...
            await rollbackManager.createHolisticSnapshot(
                'test-update-1',
                ['Analysis'],
                testProjectRoot
            );
    ...
    describe('Performance and Scalability', () => {
        test('should handle reasonable file counts efficiently', async () => {
            ...
            expect(result.success).toBe(true);
            expect(result.affectedDomains).toEqual(
                expect.arrayContaining(domains)
            );
    ...
    function createTestProjectStructure(rootPath: string): void {
        const directories = [
        ...
        fs.writeFileSync(
            path.join(rootPath, 'Lucidwonks.sln'),
            'Microsoft Visual Studio Solution File, Format Version 12.00\n'
        );

        fs.writeFileSync(
            path.join(rootPath, 'Directory.Packages.props'),
            '<?xml version="1.0" encoding="utf-8"?>\n<Project></Project>\n'
        );
    }

    function createTestCSharpFile(filePath: string, domain: string, description: string): void {
