...
import { ContextTemplateGenerator, GeneratedContextContent } from './context-template-generator.js';
import { HierarchicalRelationshipManager, HierarchyMap } from './hierarchical-relationship-manager.js';
...
export class HolisticUpdateOrchestrator {
    ...
    private readonly contextTemplateGenerator: ContextTemplateGenerator;
    private readonly hierarchicalRelationshipManager: HierarchicalRelationshipManager;
     ...
     */
    private generateGranularContextPaths(domain: string, semanticResults: SemanticAnalysisResult[]): string[] {
        const granularPaths: string[] = [];
        
        // Analyze semantic results for granular boundary qualification
        const domainSemanticResults = semanticResults.filter(result => 
            result.domainContext.includes(domain) || 
     ...
     */
    private groupResultsByGranularDomain(semanticResults: SemanticAnalysisResult[]): Map<string, SemanticAnalysisResult[]> {
        const groups = new Map<string, SemanticAnalysisResult[]>();
     ...
     */
    private evaluateGranularContextQualification(granularDomain: string, results: SemanticAnalysisResult[]): GranularContextQualification {
     ...
     */
    private assessAlgorithmicComplexity(granularDomain: string, results: SemanticAnalysisResult[]): boolean {
        ...
        const hasAlgorithmicConcepts = results.some(result => 
            result.businessConcepts.some(concept => 
     ...
     */
    private assessSemanticCoherence(granularDomain: string, results: SemanticAnalysisResult[]): boolean {
        ...
        const conceptTypes = new Set(results.flatMap(result => 
            result.businessConcepts.map(concept => concept.type)
        ));
     ...
     */
    private assessAIAssistanceValue(granularDomain: string, results: SemanticAnalysisResult[]): boolean {
        const domainLower = granularDomain.toLowerCase();
     ...
     */
    private constructGranularContextPath(granularDomain: string, qualification: GranularContextQualification): string {
     ...
     */
    private mapDomainToActualPath(granularDomain: string, _qualification: GranularContextQualification): string | null {
        // Map common domain patterns to their actual directory locations
        const domainToPathMap: { [key: string]: string } = {
     ...
     */
    private getPathConfidenceScore(granularPath: string, domainGroups: Map<string, SemanticAnalysisResult[]>): number {
     ...
     */
    private trackAdditionalGranularPaths(domain: string, additionalPaths: string[]): void {
    ...
    private async coordinateMultiLevelContextGeneration(
        ...
        const hierarchyMap = this.hierarchicalRelationshipManager.buildHierarchyMap(
            generatedContexts,
            semanticResults
        );
    ...
    private async generateParentContextWithDistribution(
        ...
        const parentContent = await this.generateEnhancedContextContent(
            parentPlan.domain, 
            domainResults, 
            'medium', // Parent contexts typically medium complexity
            'parent'
        );
    ...
    private async generateChildContextWithSpecialization(
        ...
        const childContent = await this.generateEnhancedContextContent(
            childPlan.domain, 
            subdomainResults, 
            complexityLevel,
            'child'
...
`;
     ...
     */
    private async generateEnhancedContextContent(
        domainPath: string,
        semanticResults: SemanticAnalysisResult[],
        complexityLevel: 'high' | 'medium' | 'low',
        contextType: 'parent' | 'child'
    ): Promise<string> {
        try {
            ...
            const generatedContent = await this.contextTemplateGenerator.generateContextContent(
                domainPath,
                semanticResults,
                complexityLevel
            );
...
-->`;
     ...
     */
    private determineComplexityLevel(semanticResults: SemanticAnalysisResult[]): 'high' | 'medium' | 'low' {
        if (semanticResults.length === 0) {
            return 'low';
        }
        ...
        const hasAlgorithmPatterns = semanticResults.some(result => 
            result.domainContext.includes('Analysis') || 
            result.domainContext.includes('Algorithm') ||
            result.businessConcepts.some(concept => 
                concept.name.includes('Algorithm') || 
     ...
     */
    private generateParentNavigationSection(_domainPath: string): string {
        return `
...
`;
     ...
     */
    private generateChildNavigationSection(domainPath: string): string {
...
`;
     ...
     */
    private extractGeneratedContexts(coordination: MultiLevelContextCoordination): GeneratedContextContent[] {
        const contexts: GeneratedContextContent[] = [];
    ...
    private enhanceRelationshipsWithSpecialization(
        coordination: MultiLevelContextCoordination,
        ...
        coordination.relationships.forEach(relationship => {
            ...
            if (parentContext && childContext) {
                ...
                const parentGenerated: GeneratedContextContent = {
                ...
                const childGenerated: GeneratedContextContent = {
                ...
                const specialization = this.hierarchicalRelationshipManager.generateContentSpecialization(
                    parentGenerated,
                    childGenerated,
                    semanticResults
                );
                ...
                const crossReferences = this.hierarchicalRelationshipManager.createCrossReferences(
                    parentGenerated,
                    childGenerated,
                    'parent-child',
                    semanticResults
     ...
     */
    public generateHierarchyVisualizationReport(): string {
        try {
