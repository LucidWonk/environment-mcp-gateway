...
export class BoundaryDetectionConfigManager {
    ...
    private repositoryConfigs: Map<string, RepositoryTypeConfig>;
    private defaultConfig: BoundaryDetectionConfig;
     ...
     */
    private initializeRepositoryConfigs(): void {
        // Algorithm-Heavy Repository (Financial/Trading)
        const algorithmHeavyConfig: RepositoryTypeConfig = {
        ...
        const enterpriseConfig: RepositoryTypeConfig = {
            name: 'enterprise',
        ...
        const utilityConfig: RepositoryTypeConfig = {
            name: 'utility',
     ...
     */
    async detectRepositoryType(projectRoot: string): Promise<string> {
        try {
     ...
     */
    private async analyzeRepositoryCharacteristics(projectRoot: string): Promise<{
     ...
     */
    private getDirectoriesRecursively(dir: string, maxDepth: number, currentDepth: number = 0): string[] {
     ...
     */
    private estimateConfigurationAccuracy(config: BoundaryDetectionConfig): number {
    ...
    createCustomRepositoryConfig(
        ...
        const customConfig: RepositoryTypeConfig = {
            name: typeName,
